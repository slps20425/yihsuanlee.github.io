...
sequenceDiagram
    ...
    alt Sufficient Credits
        ...
        Note over Cloud Function: Calculate cost:<br/>base_price × multiplier
...
```html
<div id="creditsCard" class="credits-card">
  <div class="credits-info">
    <span id="formCredits">Current Credits: $0 USD</span>
  </div>
  <a href="../dashboard.html" class="btn btn-primary">💳 Top-up</a>
</div>
```
...
```javascript
// Base price from Twilio API
const basePrice = 1.15; // US Local number

// Get multiplier from config
const billing = await getBillingConfig(db);
const multiplier = billing.number_multiplier || 2.0;

// Calculate user price
const userPrice = basePrice * multiplier; // $1.15 × 2.0 = $2.30
```
...
const country = 'JP';
const billing = await getBillingConfig(db);
const multiplier = billing.country_multipliers[country] || billing.common_multiplier;

// Twilio base rate: $0.05/min
const baseRate = 0.05;
const userRate = baseRate * multiplier; // $0.05 × 3.5 = $0.175/min
```
...
| Country | Base Price | Multiplier | User Price |
|---------|-----------|------------|------------|
| US Local | $1.15 | 2.0 | $2.30 |
| US Mobile | $2.00 | 2.0 | $4.00 |
| JP Local | $3.00 | 2.0 | $6.00 |
| TW Local | $2.50 | 2.0 | $5.00 |
...
|------------|-----------|------------|-----------|
| US | $0.013 | 3.0 | $0.039 |
| Japan | $0.05 | 3.5 | $0.175 |
| Taiwan | $0.04 | 3.2 | $0.128 |
| UK | $0.02 | 3.0 | $0.06 |
...
|---------|-----------|------------|-----------|
| Mouthpiece Call (2 min to US) | $0.026 | 4.0 | $0.104 |
| Restaurant Reservation (1 min to JP) | $0.05 | 4.0 | $0.20 |
| Trial Call (1 min to US) | $0.013 | 4.0 | $0.052 |
