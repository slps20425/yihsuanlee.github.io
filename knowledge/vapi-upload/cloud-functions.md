...
graph TB
    ...
    subgraph "Cloud Functions"
        Auth[lineCallbackLINE OAuth]
        Validation[validateMissionDescriptionSecurity + AI Refinement]
        Dispatcher[triggerN8nWebhookPriority Queue]
        Phone[Phone Number FunctionsSearch/Purchase/Release]
        Rates[getCallRatesPricing Info]
        Usage[getTransformedUsageHistoryBilling Data]
    end
    
    subgraph "External Services"
        LINE[LINE Login API]
        Gemini[Gemini 2.0 Flash LiteRefinement]
        OpenAI[OpenAI ModerationSafety Check]
        N8N[N8N Workflows]
        Twilio[Twilio API]
    end
    
    subgraph "Database"
        Firestore[(Firestorereservation DB)]
    end
...
sequenceDiagram
    ...
    Note over Function: ORDER BY priority DESC,reservation_utc ASC,createdAt ASC
    ...
    alt Webhook Success
        ...
        Function->>Firestore: Revert to pendingIncrement retry_count
    end
```
...
sequenceDiagram
    ...
    Function->>Function: Calculate cost(base × multiplier)
