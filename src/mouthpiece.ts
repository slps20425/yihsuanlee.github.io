...
onSnapshot(configRef, (docSnap) => {
    if (docSnap.exists()) {
        ...
        const payload = {
            ...
            userCredits: userCredits, // Current balance before deduction (backend verifies this)
            reservation_utc: serverTimestamp(), // Run now
            mission: missionEl.value,
            customMission: customMissionEl.value,
            userName: userNameEl.value,
            recipientName: recipientNameEl.value,
            targetPhoneNumber: fullPhoneNumber,
            targetPhone: fullPhoneNumber, // Legacy support
            userEmail: userEmail,
            scriptLanguage: finalLang,
            scriptContent: scriptContentEl.value,
            schedulePreference: schedulePreferenceEl.value,
            retryOneTime: retryCheck ? retryCheck.checked : false, // For UI consistency
            retry_count: (retryCheck && retryCheck.checked) ? defaultRetryCount : 0,
            createdAt: new Date().toISOString(), // Client-side time
            // Enhanced Map Data - Mouthpiece doesn't use map selection yet
            placeDetails: null
        };
